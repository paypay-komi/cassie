...
class TicTacToe {
	...
	async start() {
		...
		if (this.isAi) {
			this.channel.send(
				`Blunder Chance: ${(this.blunderChance * 100).toFixed(2)}%, Blunder Badness: ${this.blunderBadness.toFixed(2)} (0=best move, 9=random from top 10)`,
			);
		}
		...
		collector.on('collect', async (interaction) => {
			...
			if (winner) {
				...
				await interaction.editReply({
					content: `${this.currentPlayer} (${symbol}) wins!`,
					components: this.buttons,
				});
		...
		collector.on('end', async () => {
			...
			if (winner && !this.gameOver) {
				...
				await this.gameMessage.edit({
					content: `        ${this.currentPlayer} (${winner}) wins!`,
					components: this.buttons,
				});
				return;
			}
	...
	async makeAiMove() {
		...
		const bestMove = isBlunder
			? this.findBlunderMove()
			: this.findBestMove();
		if (!bestMove) return;
	...
	findBlunderMove() {
		...
		const chosen =
			moves[Math.floor(Math.random() * Math.min(topN, moves.length))];
		return chosen;
	}
