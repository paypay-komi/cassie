...
class checkersGame {
	...
	makeMoveButtons(piece) {
		...
		for (const move of validMoves) {
			buttons.push(
				new ButtonBuilder()
					.setCustomId(
						`${this.gameid}-move-${piece.row}-${piece.col}-${move.to.row}-${move.to.col}`,
					)
					.setLabel(
						`${this.convertNumberRowColToReadableRowCol(piece.row, piece.col)} → ${this.convertNumberRowColToReadableRowCol(move.to.row, move.to.col)}`,
					)
	...
	minimax(
		...
		const currentPlayer = isMaximizingPlayer
			? aiPlayer
			: aiPlayer.id === this.player1.id
				? this.player2
				: this.player1;

		const moves = activePiece
			? this.getValidMovesFromPiece(activePiece)
			: this.getAllValidMoves(currentPlayer);
		...
		if (isMaximizingPlayer) {
			...
			for (const move of moves) {
				const simulatedGame = new checkersGame(
					this.player1,
					this.player2,
					this.channel,
				);
				...
				if (move.capture) {
					...
					const additionalCaptures =
						simulatedGame.getcamptureMovesFromPiece(pieceAfterMove);
					if (additionalCaptures.length > 0)
						nextActivePiece = pieceAfterMove;
				}
	...
	async startGame() {
		...
		const filter = (interaction) =>
			interaction.customId.startsWith(this.gameid);
		const collector = this.gameMesage.createMessageComponentCollector({
			filter,
			idle: 120000,
		}); // 2 min idle
		...
		collector.on('collect', async (interaction) => {
			...
			if (action === 'move') {
				...
				const validMoves = this.getValidMovesFromPiece({
					row: fromRow,
					col: fromCol,
					piece: pieceValue,
				});
				...
				if (selectedMove.capture) {
					...
					if (additionalCaptures.length > 0) {
						...
						return interaction.update({
							...
							components: this.createPieceButtons(
								this.currentPlayer,
							),
						});
					}
	...
	isOpponentPiece(pieceValue, currentPieceValue) {
		...
		return (
			(redPieces.includes(currentPieceValue) &&
				blackPieces.includes(pieceValue)) ||
			(blackPieces.includes(currentPieceValue) &&
				redPieces.includes(pieceValue))
		);
	}
	...
	createPieceButtons(player) {
		...
		const buttons = pieces.map((piece) => {
			...
			return new ButtonBuilder()
				.setCustomId(
					`${this.gameid}-selectPiece-${piece.row}-${piece.col}-${piece.piece}`,
				)
				.setLabel(label)
				.setStyle(ButtonStyle.Primary);
		});
