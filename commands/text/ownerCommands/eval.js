...
module.exports = {
    ...
    async execute(message, args) {
        ...
        try {
            ...
            } else if (Array.isArray(result) && result.every(item => item instanceof Promise)) {
            ...
            function serializeValue(value) {
                ...
                function serializeInternal(value) {
                    ...
                    if (value instanceof Set) {
                        const setArray = Array.from(value).map(item => serializeInternal(item)); // Serialize set values
                ...
                return {
                    resultString: JSON.stringify(serializeInternal(value), (key, val) => {
            ...
            function determineFileExtension(value) {
                ...
                if (typeof value === 'bigint' || value instanceof Date || value instanceof RegExp) {
                    return 'txt';
                }
                if (value instanceof Map) {
                    return 'json';
            ...
            const sensitivePatterns = [
                {
                    pattern: /\b[mM][a-zA-Z0-9_-]{23}\.[a-zA-Z0-9_-]{6}\.[a-zA-Z0-9_-]{27}\b/g,
                    label: '(redacted: Discord Bot token)',
                },
                {
                    pattern: /bearer\s+[A-Za-z0-9_\-\\.~:/?#[\]@!$&'()*+,;=]{20,}/ig,
                ...
                {
                    pattern: /\bhttps:\/\/discord(app)?\.com\/api\/webhooks\/[0-9]+\/[A-Za-z0-9_-]+\b/g,
            ...
            function redactSensitiveInformation(inputString) {
                ...
                sensitivePatterns.forEach(({ pattern, label }) => {
                    ...
                    for (const match of matches) {
                        ...
                        sensitivePatterns.forEach(({ pattern: innerPattern, label: innerLabel }) => {
                            // Apply inner pattern to the current match
                            const redactedMatch = match[0].replace(innerPattern, innerLabel);
                            const redactedLength = match[0].length - redactedMatch.length;
                        ...
                        redactedString = redactedString.replace(match[0], bestRedaction);
                    }
                });
            ...
            if (resultStringLength > 1980) {
                ...
                const uploadMessage = await message.channel.send('The result is too large to send as a message and is being uploaded as a file. 📤');
                ...
                await message.channel.send(`\`\`\`${fileExtension}\n${redactedString}\n\`\`\``);
            }
        } catch (error) {
            // Log the error
            ...
            if (errorStringLength > 1980) {
                // Send an initial message indicating that the upload is in progress
                const uploadMessage = await message.channel.send('The error message is too large to send as a message and is being uploaded as a file. 📤');
