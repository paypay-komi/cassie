	...
	async execute(message, args) {
		...
		if (!KEYS.includes(key))
			return message.reply(
				v2(`Unknown key \`${key}\`. Available: ${KEYS.join(", ")}`),
			);

		const val = args.slice(1).join(" ");
		if (!val)
			return message.reply(
				v2(`Usage: \`c.economy config ${key} <value>\``),
			);
		...
		if (
			...
			if (key === "taxRate" && (num < 0 || num > 100))
				return message.reply(v2("Tax rate must be 0–100."));
			parsed[key] = num;
		} else {
			parsed[key] = val;
		}
