...
module.exports = {
    ...
    async execute(message, args) {
        ...
        const allCommands = [...textCommands.values()].sort((a, b) => a.name.localeCompare(b.name));
        const pageSize = 3;
        const totalPages = Math.max(1, Math.ceil(allCommands.length / pageSize));
        let page = 0;

        const generateEmbed = (pageIndex) => {
            const slice = allCommands.slice(pageIndex * pageSize, (pageIndex + 1) * pageSize);
            return new EmbedBuilder()
                .setTitle('Available Commands')
                .setDescription(slice.map((c, i) => renderCommand(c, 0, i === slice.length - 1)).join('\n'))
                .setColor('#00AEEF')
        ...
        collector.on('collect', async interaction => {
            if (interaction.user.id !== message.author.id) {
                return interaction.reply({ content: "🚫 You can't control this help menu.", ephemeral: true });
            }
            ...
            switch (action) {
                ...
                case 'next': page = Math.min(page + 1, totalPages - 1); break;
