...
/**
 * Get all valid command invocation names (canonical paths + aliases).
 * @returns {string[]} Sorted list of names
 */
function getAllCommandNames(client) {
	ensureNameMap(client);
	return [...client._cmdNameMap.keys()].sort();
}

/**
 * Shared autocomplete helper for the "command" option on manage subcommands.
 * Returns command names/aliases that match the user's partial input, plus "all".
 *
 * @param {string} value User's partial input
 * @param {import("discord.js").Client} client
 * @returns {{name: string, value: string}[]} Up to 25 choices
 */
function suggestCommandNames(value, client) {
	const all = ["all", ...getAllCommandNames(client)];
	const filtered = value
		? all.filter((name) => name.includes(value.toLowerCase()))
		: all;
