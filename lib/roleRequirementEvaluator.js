...
const log = getLogger("roleRequirementEvaluator");
		...
		if (token.value === "(") {
			...
			if (!closing || closing.value !== ")") {
				throw createError(
					...
					`Expected ")" but got ${
						closing ? `"${closing.value}"` : "end of expression"
					}`,
					closing
...
function tokenize(expression) {
	...
	while (position < expression.length) {
		...
		// Numbers
		const numberMatch = expression.slice(position).match(/^\d+(?:\.\d+)?/);
		...
		if (identifierMatch) {
			...
			if (rawValue.toUpperCase() === LOGICAL_OPERATORS.AND) {
				value = LOGICAL_OPERATORS.AND;
			} else if (rawValue.toUpperCase() === LOGICAL_OPERATORS.OR) {
...
async function handleRoleAssignment(guild, memberId) {
	if(!guild || !memberId) {
		log.error("handleRoleAssignment called with invalid guild or memberId", { guild, memberId });
		return;
	}
	const guildMemberActivity = await prisma.guildMemberActivity.findUnique({
		where: {
			guildId_userId: {
				guildId: guild.id,
				userId: memberId,
	...
	if (!guildMemberActivity) return;
	const roles = await prisma.guildMemberRoleRequirement.findMany({
		where: {
			guildId: guild.id,
		},
	});
	...
	const voiceSeconds = guildMemberActivity.voiceSeconds;
	const daysInServer = Math.floor(
		(Date.now() - member.joinedTimestamp) / (1000 * 60 * 60 * 24),
	);
	const messageCount = guildMemberActivity.messageCount;
	for (const role of roles) {
		if (memberRoles.includes(role.roleId)) continue;
		const ast = role.ast;
		const isEligible = evaluateRequirement(ast, {
			voiceSeconds,
			daysInServer,
			messageCount,
		});
		if (isEligible) {
			try {
				...
				log.error(
					`Failed to assign role ${role.roleId} to member ${member.id} in guild ${guild.id}:`,
					error,
				);
			}
		}
