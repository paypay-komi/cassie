// Handles reaction role ADD — assigns role when user reacts
const { Events } = require("discord.js");
const { getLogger } = require("../lib/logger");
const log = getLogger("reactionRoleAdd");
const db = require("../db");
...
module.exports = {
	...
	description: "Handles reaction role add",
	async execute(client, reaction, user) {
		...
		try {
			...
			log.debug(`Added role ${entry.roleId} to ${user.tag}`);
		} catch (err) {
			log.error(`Failed to add role ${entry.roleId} to ${user.tag}:`, err);
		}
	},
};
