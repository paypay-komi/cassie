...
function resolveNested(command, args) {
    ...
    while (args.length && current.subcommands) {
        ...
        const next =
            ...
            Object.values(current.subcommands).find(sc => sc.aliases?.includes(input));
...
module.exports = {
    ...
    async execute(client, message) {
        ...
        if (!commandName) {
            const suggestion = didYouMean(input + ' ' + args.join(' '), message.client.textCommands);
            return message.reply(`That's not a valid command! Did you mean: ${suggestion}?`);
        }
